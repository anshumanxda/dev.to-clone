...
const PodcastCard = ({ data }) => {
  return (
    <a target='_blank' rel="noreferrer" href={`https://dev.to${data.path}`}>
      <div className='flex gap-3 border border-gray-200 bg-white p-6 items-center my-2 rounded-md '>
        ...
        <div>
          ...
          <p className='text-lg lg:text-xl font-semibold text-gray-900'>{data?.podcast?.title}</p>
        </div>
      </div>
    </a>
  )
